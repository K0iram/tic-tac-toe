...
/***/ function(module, exports, __webpack_require__) {
	...
	var gamesetEvents = __webpack_require__(12);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var store = __webpack_require__(10);
	var ticTacToe = __webpack_require__(15);
	var PlayerOne = 'X';
	var PlayerTwo = 'O';
	...
	var onShowGame = function onShowGame(event) {
	  var data = getFormFields(event.target);
	  event.preventDefault();
	  api.showGame(data.game.id).then(ui.success).catch(ui.failure);
	};

	var onIndexGame = function onIndexGame(event) {
	  event.preventDefault();
	  api.indexGames().then(function (response) {
	    $('.showgames').text("You have played " + response.games.length + " games!!");
	  }).catch(ui.failure);
	};

	var OnUpdateGame = function OnUpdateGame(position, currentTurn, gameOver) {
	  event.preventDefault();
	  api.updateGame(position, currentTurn, gameOver).then(function (response) {
	    store.game = response.game;
	  }).then(ui.success).catch(ui.failure);
	};
